...
点击画布底部全屏按钮左侧的画笔按钮进入标注模式。工具栏支持文字、自由笔迹、方框、箭头，以及颜色、粗细和字号设置；标注随实验板缩放和平移。

- 文字工具点击画布后输入，Enter 换行，点击“完成”或 Ctrl/Cmd+Enter 提交，Esc 取消。
- 标注时鼠标左键或单指用于绘制，中键平移、滚轮缩放和双指手势继续可用；输入文字期间暂停画布手势。
- 底部撤销/重做及 Ctrl/Cmd+Z、Ctrl/Cmd+Shift+Z 只操作当前模式的历史；清空标注也可撤销。
- 再次点击标注按钮或选择电路工具退出。Esc 依次取消未完成操作、退出标注、退出网页全屏。退出后标注仍显示，可以操作其下的元件；标注期间仿真继续运行。
- 标注仅用于当前课堂，不保存到本地草稿或云项目；刷新、新建或打开项目会清空。已有标注不支持移动或修改；使用清空左侧的橡皮擦，可点击或拖动擦除整条笔迹、方框、箭头或文字，一次拖动记为一步撤销。

## 外置传感器与模块

元件库的“芯片传感器模型”包含 ESP32-S3 开发板以及 DHT11、HC-SR04、红外避障、光敏、声音、热敏、MQ2、直流电机、SG90、红绿灯、单路继电器、震动模块、旋钮电位器和无源压电蜂鸣器。ESP32-S3 和无源压电蜂鸣器放置在面包板孔位上；蜂鸣器默认两脚间隔两个孔距（中间空一孔）。其余模型为外置模块。它们只展示实物模型和接线，不产生测量值，也不参与 CircuitJS 求解。
